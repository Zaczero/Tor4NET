...
namespace Tor.Events
{
    ...
    {
        private readonly static Dictionary<Enumerators.Event, Type> dispatchers;
        private readonly static string EOL = "\r\n";

        private readonly Client client;
        private readonly Dictionary<Enumerators.Event, int> events;
        ...
        static Events()
        {
            dispatchers = new Dictionary<Enumerators.Event, Type>();

            Enumerators.Event[] values = Enum.GetValues(typeof(Enumerators.Event)) as Enumerators.Event[];
            ...
            foreach (Type type in Assembly.GetExecutingAssembly().GetTypes().Where(t => t.Namespace == "Tor.Events" && typeof(Dispatcher).IsAssignableFrom(t)))
            {
                ...
                foreach (Enumerators.Event value in values)
                {
        ...
        internal Events(Client client)
        {
            ...
            this.disposed = false;
            this.enabled = true;
            this.events = new Dictionary<Enumerators.Event, int>();
            this.multiLine = null;
            this.socket = null;
        ...
        {
            add { bandwidthChangedHandlers += value; AddEvent(Enumerators.Event.Bandwidth); }
            remove { bandwidthChangedHandlers += value; RemoveEvent(Enumerators.Event.Bandwidth); }
        }
        ...
        {
            add { circuitChangedHandlers += value; AddEvent(Enumerators.Event.Circuits); }
            remove { circuitChangedHandlers -= value; RemoveEvent(Enumerators.Event.Circuits); }
        }
        ...
        {
            add { configurationChangedHandlers += value; AddEvent(Enumerators.Event.ConfigChanged); }
            remove { configurationChangedHandlers -= value; RemoveEvent(Enumerators.Event.ConfigChanged); }
        }
        ...
        {
            add { debugReceivedHandlers += value; AddEvent(Enumerators.Event.LogDebug); }
            remove { debugReceivedHandlers -= value; RemoveEvent(Enumerators.Event.LogDebug); }
        }
        ...
        {
            add { errorReceivedHandlers += value; AddEvent(Enumerators.Event.LogError); }
            remove { errorReceivedHandlers -= value; RemoveEvent(Enumerators.Event.LogError); }
        }
        ...
        {
            add { infoReceivedHandlers += value; AddEvent(Enumerators.Event.LogInfo); }
            remove { infoReceivedHandlers -= value; RemoveEvent(Enumerators.Event.LogInfo); }
        }
        ...
        {
            add { noticeReceivedHandlers += value; AddEvent(Enumerators.Event.LogNotice); }
            remove { noticeReceivedHandlers -= value; RemoveEvent(Enumerators.Event.LogNotice); }
        }
        ...
        {
            add { orConnectionChangedHandlers += value; AddEvent(Enumerators.Event.ORConnections); }
            remove { orConnectionChangedHandlers -= value; RemoveEvent(Enumerators.Event.ORConnections); }
        }
        ...
        {
            add { streamChangedHandlers += value; AddEvent(Enumerators.Event.Streams); }
            remove { streamChangedHandlers -= value; RemoveEvent(Enumerators.Event.Streams); }
        }
        ...
        {
            add { warnReceivedHandlers += value; AddEvent(Enumerators.Event.LogWarn); }
            remove { warnReceivedHandlers -= value; RemoveEvent(Enumerators.Event.LogWarn); }
        }
        ...
        private void SetEvents()
        {
            ...
            lock (synchronize)
            {
                ...
                foreach (KeyValuePair<Enumerators.Event, int> interested in events)
                {
                    string name = ReflectionHelper.GetDescription<Enumerators.Event>(interested.Key);
